...
// One-off bulk-importer that reads all configured MDB sources and merges their
// rows into the LawPortalCPiR10 SQL database with appropriate Systems= tags.
// Uses the same exe with the "read" verb to read MDBs in a 32-bit subprocess.
internal static class Importer
{
    public static async Task<int> Run(string[] args)
    {
        ...
        // Self-invoke: spawn another LawPortal.Mdb.exe with "read <mdb>" to do the 32-bit JET work.
        var selfExe = Environment.ProcessPath;
        if (string.IsNullOrEmpty(selfExe) || !File.Exists(selfExe))
        {
            Console.Error.WriteLine($"Cannot locate self exe at: {selfExe}");
            return 1;
        }
        ...
        try
        {
            ...
            foreach (var imp in imports)
            {
                ...
                var mdbData = await ReadMdb(selfExe, imp.Path);
    ...
    private static async Task<List<string>> GetTablesWithSystemsColumn(SqlConnection conn)
    {
    ...
    // Spawn a child LawPortal.Mdb.exe with "read <mdbPath>" and parse its JSON stdout.
    // Same pattern the comparison service uses; it isolates the 32-bit OLE call
    // and avoids leaking JET handles into the importer.
    private static async Task<Dictionary<string, List<Dictionary<string, JsonElement>>>?> ReadMdb(string selfExe, string mdbPath)
    {
    ...
    private static object? GetSqlValue(JsonElement el)
    {
    ...
    private static async Task ImportPatR10v22(SqlConnection destConn)
    {
