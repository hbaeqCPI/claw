...
namespace LawPortal.Web.Areas.Trademark.Controllers
{
    ...
    public class AreaCountryDeleteController : BaseController
    {
        ...
        public async Task<IActionResult> PageRead([DataSourceRequest] DataSourceRequest request, List<QueryFilterViewModel> mainSearchFilters)
        {
            ...
            var result = await data.ToNavigableDataSourceResultAsync(request, "areaCode:Area", "country:Country", "areaNewCode:AreaNew", "countryNew:CountryNew", "systems:Systems");
            return Json(result);
        }
        ...
        public async Task<IActionResult> Detail(string areaCode, string country, string areaNewCode, string countryNew, string systems = "", bool singleRecord = false, bool fromSearch = false)
        {
            ...
            var model = new PageViewModel
            {
                ...
                RecordId = 1,
                RecordKey = RecordNavigationKey.Build(("areaCode", areaCode), ("country", country), ("areaNewCode", areaNewCode), ("countryNew", countryNew), ("systems", systems)),
        ...
        public async Task<IActionResult> Delete(string areaCode = "", string country = "", string areaNewCode = "", string countryNew = "", string systems = "")
        {
            var count = await _repository.Database.ExecuteSqlRawAsync(
                "DELETE FROM tblTmkAreaCountryDelete WHERE Area=@p0 AND Country=@p1 AND Systems=@p4",
                new object[] {
        ...
        {
            return await GetPicklistData(_repository.TmkAreaCountryDeletes.AsNoTracking(), request, property, text, filterType, requiredRelation);
        }
