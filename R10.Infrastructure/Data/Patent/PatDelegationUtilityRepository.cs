...
{
    public class PatDelegationUtilityRepository : IPatDelegationUtilityRepository
    {
        protected readonly ApplicationDbContext _dbContext;

        public PatDelegationUtilityRepository(ApplicationDbContext dbContext)
        {
            _dbContext = dbContext;
        }

        public IQueryable<CPiUser> CPiUsers => _dbContext.CPiUser.AsNoTracking();
        public IQueryable<PatDueDateDelegation> PatDueDateDelegations => _dbContext.PatDueDateDelegations.AsNoTracking();
        ...
        public IQueryable<DelegationUtilityPreviewDTO> GetPreviewList(DelegationUtilityCriteriaDTO searchCriteria)
        {
            ...
            {
                sourceQuery = "Select * From vwPatActionDueDateDelegation";
            ...
            {
                sourceQuery = @"Select a.CaseNumber,a.Country,a.SubCase, a.ActId, d.DDId as Id, d.DueDate,a.ActionType,d.ActionDue,d.Indicator, '' as DelegatedUser, '' as DelegatedGroup, u.Id as UserId, grp.Id as GroupId 
                               From tblPatActionDue AS a WITH (NOLOCK) 
                               INNER JOIN tblPatDueDate AS d WITH (NOLOCK) ON d.ActId = a.ActId
                               LEFT JOIN  tblPatDueDateDelegation AS ddd WITH (NOLOCK) ON d.DdId = ddd.DdId 
        ...
        public async Task<DelegationUtilityResultDTO> RunUpdate(string updateMode, int[] ids, string[] delegateTo,string userName,string? fromUser, int fromGroup, bool reassign)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("procPatDelegationUtility"))
            {
