...
{
    public class ComboBoxOwner : ViewComponent
    {
                                           ...
                                           int listWidth = 300, bool showLinkButton = false, bool limitToList = true)
        {
            ...
            var model = new ComboBoxOptions
            {
                ...
                Controller = string.IsNullOrEmpty(controller) ? "Owner" : controller,
                Action = string.IsNullOrEmpty(action) ? "GetOwnersList" : action,
                ...
                ListWidth = listWidth,
                LinkButtonUrl = showLinkButton ? Url.Action("DetailLink", "Owner", new { Area = "Shared", ownerId = "actualValue" }) : "",
