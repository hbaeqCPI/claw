...
export default class RelatedTrademarkPage {
    ...
    initialize = (gridName, id) => {
        $(document).ready(() => {
            ...
            const relatedTrademarksGrid = $(`#${gridName}`);

            if (relatedTrademarksGrid) {
                relatedTrademarksGrid.find(".k-grid-toolbar").on("click",
                    ".k-grid-Add",
                    () => {
                        const url = $(`#${gridName}`).parent().data("url-add-related-trademark");
                        const data = {
                            parentId: id,
                        };

                        this.openTrademarkPopup("relatedTrademarkSelectionGrid", gridName, "relatedTrademarkAddDialog", id, 0, url, data, true);
                    })
            }
        });
    ...
    // #region related trademark page
    relatedTrademarksGridDataBound = (gridName, id) => {
        const relatedTrademarksGrid = $(`#${gridName}`);
    ...
    editRelatedTrademarksGridRow = (e, gridName) => {
        const grid = $(`#${gridName}`).data("kendoGrid");
        const dataItem = grid.dataItem($(e.currentTarget).closest("tr"));

        const url = $(`#${gridName}`).parent().data("url-edit-related-trademark");
        const data = {
            keyId: dataItem.KeyId,
            parentId: dataItem.ParentId
        };

        this.openTrademarkPopup("relatedTrademarkSelectionGrid", gridName, "relatedTrademarkAddDialog", dataItem.ParentId, dataItem.KeyId, url, data, true);
    }

    getRelatedTrademarkSources = () => {
        const grid = $("#relatedTrademarkSelectionGrid").data("kendoGrid");
        grid._selectedIds = {};
        grid.clearSelection();
    ...
    openTrademarkPopup = (gridName, parentGridName, dialogContainerName, parentId, keyId, url, data, closeOnSave) => {
        const self = this;
    ...
    getRrelatedTrademarkParam = (dialogContainerName) => {
        ...
        const param = {
            ...
            regNumber: form.find("input[name = 'RegNumber']").data("kendoComboBox").text(),
            classIdList: form.find("select[name = 'ClassId']").data("kendoMultiSelect").value(),
            goods: form.find("input[name = 'Goods']").val(),
            trademarkName: form.find("input[name = 'TrademarkName']").data("kendoComboBox").text(),
    ...
    onTrademarkReplaceGridDataBound = (e) => {
