export default class DocViewer {
    constructor() {
        ...
        this.collapsedFoldersStorageName = "doc-viewer-collapsedFolders";
    ...
    initializeViewer = (activePage) => {
        ...
        const docViewer = this.page.find(".document-viewer");
        if (docViewer.length > 0) {
            const displayOption = docViewer.find('.display-option input[name="DocumentDisplayOption"]');
            const searchForm = docViewer.find(".documents .search .form");
    ...
    initializeToolbar = () => {
        const filters = this.page.find(".document-viewer .documents .search");
        const toolbar = this.page.find(".document-viewer .documents .toolbar .k-toolbar");
    ...
    getDefaultImage = () => {
        const defaultImage = this.page.find(".doc-viewer-default-image");
    ...
    getDefaultGridImage = (activePage) => {
        ...
        this.page.find(".doc-viewer-default-image:not(.loaded)").each((i, e) => {
            const el = $(e);
    ...
    getFolderTree = (containerId, callback) => {
        ...
        const docViewer = this.page.find(".document-viewer");
        const sidebar = docViewer.find(".side-bar");
        const documents = docViewer.find(".target");

        cpiLoadingSpinner.show();
        $.get(sidebar.data("url"), { pageId: this.page.attr("id"), containerId: containerId, documentLink: docViewer.data("document-link"), token: docViewer.data("token") })
            .done((response) => {
                cpiLoadingSpinner.hide();
                if (response) {
    ...
    getViewer = (displayOption) => {
        const docViewer = this.page.find(".document-viewer");
        const viewer = docViewer.find(".documents .viewer");

        cpiLoadingSpinner.show();
        $.get(viewer.data("url"), { pageId: this.page.attr("id"), displayOption: displayOption, token: docViewer.data("token") })
            .done((response) => {
                cpiLoadingSpinner.hide();
    ...
    getSetup = () => {
        const docViewer = this.page.find(".document-viewer");
        const folderTree = docViewer.find(".folder-tree");

        cpiLoadingSpinner.show();
        $.get(folderTree.data("setup-url"), { documentLink: docViewer.data("document-link"), token: docViewer.data("token") })
            .done((response) => {
                cpiLoadingSpinner.hide();
    ...
    initializeSetup = () => {
        ...
        setup.find(".save").on("click", (e) => {
            ...
            if (url) {
                ...
                $.post(url, { documentLink: docViewer.data("document-link"), rootContainerId: rootContainerId, defaultFolderId: defaultFolderId, token: docViewer.data("token"), __RequestVerificationToken: this.getVerificationToken() })
                    .done(() => {
                        cpiLoadingSpinner.hide();
        ...
        setup.find(".btn-new-workspace").on("click", (e) => {
            ...
            if (url) {
                ...
                $.get(url, { documentLink: docViewer.data("document-link"), token: docViewer.data("token") })
                    .done((response) => {
                        cpiLoadingSpinner.hide();
                        cpiConfirm.save(title, response, () => {
                            ...
                            cpiLoadingSpinner.show();
                            $.post(url, { documentLink: docViewer.data("document-link"), token: docViewer.data("token"), workspaceName: workspaceName })
                                .done((response) => {
                                    cpiLoadingSpinner.hide();
    ...
    //Opens third party storage (iManage, NetDocs, etc) login screen in new tab
    ...
    getAuthToken = (callback) => {
        const docViewer = this.page.find(".document-viewer");
        if (docViewer.length > 0 && docViewer.data("auth-url")) {
            const statusKey = "doc-viewer-signin";
            const checkAuth = () => {
            ...
            if (this.authenticating <= 1)
                window.open(docViewer.data("auth-url"));
        }
        else {
            console.error("Document storage authentication URL is undefined.")
        }
    }

    //Document storage authentication error handler
    checkError(error, callback) {
        //Authorization Service Filter returns 401 error if user has no access token for document storage API
        if (error.status == 401 && callback)
            //Open document storage sign-in endpoint in new tab to acquire access token
    ...
    selectFolder = (e) => {
        ...
        const searchForm = this.page.find(".document-viewer .documents .search .form");
    ...
    getKendoDocumentViewer = () => {
        const searchResults = this.page.find(".document-viewer .documents .viewer .results");
    ...
    nameReadData = () => {
        const folderTree = this.page.find(".document-viewer .folder-tree");
        return { containerId: folderTree.data("root") };
    }

    defaultFolderReadData = () => {
        const setup = this.page.find(".document-viewer .folder-tree .setup");
    ...
    viewerReadData = (e) => {
        const searchForm = this.page.find(".document-viewer .documents .search .form :input");
    ...
    documentsOnDragOver = (event) => {
        ...
        //no active folder
        if (this.page.find(".document-viewer .folder-tree").find(".folder.active").length == 0)
            return;
    ...
    documentOnClick = (event) => {
        ...
        const docViewer = this.page.find(".document-viewer");
        const documents = docViewer.find(".documents");
        const url = documents.data("viewer-url");

        cpiLoadingSpinner.show()
        $.get(url, { id: el.data("document-id"), fileName: el.text(), token: docViewer.data("token") })
            .done((result) => {
                cpiLoadingSpinner.hide();
    ...
    documentOnDragStart = (event) => {
        ...
        const dragImage = this.page.find(".document-viewer .drag-image");
    ...
    dropOnUpload = (event, destinationFolderId) => {
        const folders = this.page.find(".document-viewer .folder-tree .folders");
    ...
    moveDocuments = (event) => {
        ...
        if (url) {
            cpiLoadingSpinner.show();
            $.post(url, { ids: ids, destinationFolderId: destinationFolder.data("folder"), token: docViewer.data("token"), __RequestVerificationToken: this.getVerificationToken() })
                .done(() => {
                    cpiLoadingSpinner.hide();
    ...
    dropFolder = (event) => {
        ...
        if ((folder.closest("li").find(`.folder[data-folder='${destinationFolder.data("folder")}']`).length > 0) || !destinationFolder.hasClass("breeder")) {
        ...
        cpiLoadingSpinner.show();
        $.post(folderTree.data("move-url"), { folderId: folder.data("folder"), destinationFolderId: destinationFolder.data("folder"), token: docViewer.data("token"), __RequestVerificationToken: this.getVerificationToken() })
            .done(() => {
                cpiLoadingSpinner.hide();
    ...
    folderMenuOnOpen = (e) => {
        ...
        const isDefault = folder.data("folder") == this.page.find(".document-viewer .folder-tree").data("default");

        menu.enable("#delete", isFolder && !hasDocuments && !hasSubfolders);
        menu.enable("#rename", isFolder);
        menu.enable("#default", isFolder && !isDefault);
        menu.enable("#create", folder.hasClass("breeder"));
    }
    ...
    folderMenuOnSelect = (e) => {
        ...
        const data = { id: folder.data("folder"), documentLink: docViewer.data("document-link"), token: docViewer.data("token"), __RequestVerificationToken: this.getVerificationToken() };
    ...
    getVerificationToken = () => {
        const form = this.page.find(".document-viewer").closest("form");
    ...
    gridOnCommand = (e) => {
        ...
        switch (e.data.commandName) {
            case "Edit":
                this.saveDocument(dataItem.Id, dataItem.Title, this.page.find(".document-viewer .documents .toolbar .k-toolbar .save-document.edit").data("url"), dataItem.ContainerId);
                break;
    ...
    saveDocument = (id, title, url, folderId) => {
        ...
        if (url) {
            cpiLoadingSpinner.show();
            $.get(url, { docId: id, folderId: folderId || activeFolder.data("folder"), token: docViewer.data("token") })
                .done((response) => {
                    cpiLoadingSpinner.hide();
    ...
    deleteDocument = (dataItem) => {
        ...
        if (url) {
            this.confirmDelete(url,
                ...
                { id: dataItem.Id, token: docViewer.data("token"), __RequestVerificationToken: this.getVerificationToken() }
            );
        }
    }
    ...
    deleteDocuments = () => {
        ...
        if (url) {
            this.confirmDelete(url, pageHelper.formatString(documents.data("delete-multi-confirm"), this.selected.length),
                { ids: ids, token: docViewer.data("token"), __RequestVerificationToken: this.getVerificationToken() }
            );
        }
    }
    ...
    mergeDocuments = () => {
        ...
        const folderId = activeFolder.data("folder");
            ...
            .done((result) => {
                ...
                container.on("click", "#mergeDocument", (e) => {
                    ...
                    if (mergedDocName > "") {
                        ...
                        cpiLoadingSpinner.show();
                        $.post(mergeUrl, { folderId: folderId, mergedDocName: mergedDocName, docList: orderedDocs })
    ...
    getDelay = () => {
        //iManage include_subtree=true returns STALE data. use this.refreshDelay as workaround
        const searchForm = this.page.find(".document-viewer .documents .search .form");
