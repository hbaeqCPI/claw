...
export default class TmkWorkflowPage extends ActivePage {
    constructor() {
        super();
    }
    ...
    initialize = (screen, id) => {
        this.editableGrids = [            
            { name: "tmkWorkflowActionGrid", filter: { parentId: id }, afterSubmit: this.updateRecordStamps },           
            { name: "tmkWorkflowActionParametersGrid", filter: { parentId: id }, afterSubmit: this.updateRecordStamps }           
        ];
    ...
    tabChangeSetListener = () => {
        ...
        $(document).ready(function() {
            const actionsGrid = $("#tmkWorkflowActionGrid");
            ...
            actionsGrid.on("click", ".attachment-filter-marker", (e) => {
                ...
                const url = `${baseUrl}/Trademark/Workflow/AttachmentFilterLoad`;
    ...
    triggerTypeSearchOnChange = (e) => {
        ...
        if (selectedItem) {
            var triggerValue = $("#TriggerTypeId_tmkWorkflowSearch").data("kendoComboBox");
    ...
    getTriggerType() {
        return {
            ...
            screenType: $("#tmkWorkflowDetailsView").find("#ScreenName").val()
        };
    }
    ...
    getTriggerTypeIdForSearch = () => {
        return {
            triggerType: $("#TriggerTypeId_tmkWorkflowSearch").data("kendoComboBox").value()
        };
    }
    ...
    triggerTypeOnChange = (e) => {
        ...
        //trigger value fields != fileuploaded, recorddeleted, attorneymodified, actiondelegated+
        if (!(selectedItem.Value == 3 || selectedItem.Value == 5 || selectedItem.Value == 7 || selectedItem.Value == 8 || selectedItem.Value == 9 ||
            selectedItem.Value == 10 || selectedItem.Value == 11 || selectedItem.Value == 12 || selectedItem.Value == 20)) {
        ...
        //recorddeleted, attymodified,actiondelegated+
        else if (selectedItem.Value == 5 || selectedItem.Value == 7 || selectedItem.Value == 8 || selectedItem.Value == 9 ||
            selectedItem.Value == 10 || selectedItem.Value == 11 || selectedItem.Value == 12 || selectedItem.Value == 20) {
        ...
        if (selectedItem.Value == 3 || selectedItem.Value == 5 || selectedItem.Value == 13) {
    ...
    actionTypeOnChange = (e) => {
        if (e.item) {
            ...
            const grid = $("#tmkWorkflowActionGrid").data("kendoGrid");
    ...
    getActionType = (e) => {
        var grid = $("#tmkWorkflowActionGrid").data("kendoGrid");
