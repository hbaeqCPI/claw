...
export default class TmkCECountrySetup extends ActivePage {
    ...
    tabChangeSetListener = () => {
        const self = this;
        $('#tmkCECountrySetup-tab a').on('click',
            (e) => {
                e.preventDefault();
    ...
    getCountryFromCopyScreen = () => {
        const country = $("#tmkCECountrySetupCopyDialog").find("input[name='Country']");
        return { country: country.val() };
    }

    showCopyScreen() {
        const popupContainer = $(".site-content .popup").last();
        const dialogContainer = popupContainer.find("#tmkCECountrySetupCopyDialog");
    ...
    mainGridEdit(e) {
        ...
        var elements = {
            ...
            markTypeMultiSelect: { input: editorContainer.find("#MarkTypeList").data("kendoMultiSelect") },
            ...
            tmkCPICostLabel: { divElement: editorContainer.find('#tmkCPiCostLabel') },        
        ...
        tmkCECountrySetupPage.hideAllElements(elements); // Hide all elements first

        // --- 3. CPIADMIN READ-ONLY STATE ---
        if (model.CPICost && !tmkCECountrySetupPage.isCPIAdmin) {
            elements.tmkCPICostLabel.divElement.show();
        ...
        switch (model.DataType) {
            case tmkCECountrySetupPage.CostDataType.STRING:
                elements.cost.divElement.show();
                break;
            case tmkCECountrySetupPage.CostDataType.DATE:
                ...
                break;
            case tmkCECountrySetupPage.CostDataType.DATERANGE:
                ...
                break;
            case tmkCECountrySetupPage.CostDataType.NUMERIC:                
                ...
                break;
            case tmkCECountrySetupPage.CostDataType.BOOLEAN:
        ...
        if (model.MarkType) {
            var markTypeList = model.MarkType.split("|").map(item => item.trim()).filter(item => item !== "");
            if (markTypeList) {
                elements.markTypeMultiSelect.input.value(markTypeList);
            }
        }
        ...
        elements.translationType.input.bind("change", function(event) {
            tmkCECountrySetupPage.mainGridEditorTranslationTypeChange({
                data: {
        ...
        const currentTranslationType = model.TranslationType == tmkCECountrySetupPage.TranslationType.PAGES.Value ? tmkCECountrySetupPage.TranslationType.PAGES.Text : model.TranslationType == tmkCECountrySetupPage.TranslationType.WORDS.Value ? tmkCECountrySetupPage.TranslationType.WORDS.Text : tmkCECountrySetupPage.TranslationType.PAGES.Text;        
        tmkCECountrySetupPage.mainGridEditorTranslationTypeChange({
            data: {
        ...
        elements.costTypeDropDown.input.bind("change", function(event) {
            tmkCECountrySetupPage.mainGridEditorCostTypeChange({
                data: {
                    sender: event.sender,                    
        ...
        elements.dataTypeDropDown.input.bind("change", function(event) {
            tmkCECountrySetupPage.mainGridEditorDataTypeChange({
                data: {
                    sender: event.sender,                    
                    editorContainer: editorContainer
                }
    ...
    mainGridSave(e) {
        ...
        switch (model.DataType) {
            case tmkCECountrySetupPage.CostDataType.STRING:
            case tmkCECountrySetupPage.CostDataType.SELECTION:
            case tmkCECountrySetupPage.CostDataType.NUMERICRANGE:
        ...
        var isNumericTranslation = model.DataType === tmkCECountrySetupPage.CostDataType.NUMERIC && model.CostType && model.CostType.toLowerCase() === 'translation';
        ...
        } else if (model.DataType !== tmkCECountrySetupPage.CostDataType.STRING
            && model.DataType !== tmkCECountrySetupPage.CostDataType.SELECTION 
            && model.DataType !== tmkCECountrySetupPage.CostDataType.NUMERICRANGE 
        ...
        if (model.MarkTypeList) {
            model.MarkType = model.MarkTypeList.map(item => item.MarkType).join("|");            
        }
    }
    ...
    mainGridDataBound(e) {
        ...
        grid.tbody.find("tr[role='row']").each(function () {
            ...
            if (row.DataType === tmkCECountrySetupPage.CostDataType.NUMERIC) {
                var cell = $(this).children().eq(columnIndex);
                cell.html(row.Opts + " " + row.DefaultValue);
            }
        
            // 2. Disable hierarchy grid based on data type            
            if (row.DataType !== tmkCECountrySetupPage.CostDataType.SELECTION 
                && row.DataType !== tmkCECountrySetupPage.CostDataType.BOOLEAN 
                && row.DataType !== tmkCECountrySetupPage.CostDataType.NUMERICRANGE) {
                $(this).find(".k-hierarchy-cell a").remove();
            }
        
            // 3. Persist expanded rows
            if (tmkCECountrySetupPage.parentGridExpandedRows && row) {
                if ($.inArray(row.CostId, tmkCECountrySetupPage.parentGridExpandedRows) >= 0) {
                    grid.expandRow($(this)); 
                }
            }
        });
    ...
    mainGridDetailExpand(e) {
        ...
        if (dataItem) {
            tmkCECountrySetupPage.parentGridExpandedRows.push(dataItem.CostId);
        }
    }
    ...
    mainGridDetailCollapse(e) {
        let dataItem = this.dataItem(e.masterRow);
        if (dataItem) {
            tmkCECountrySetupPage.parentGridExpandedRows = tmkCECountrySetupPage.parentGridExpandedRows.filter(id => id !== dataItem.CostId);
        }
    }            
    ...
    mainGridEditorCostTypeChange(e) {
        ...
        if (selectedCostType) {
            ...
            if (elements.dataTypeDropDown.input && elements.dataTypeDropDown.input.value() === tmkCECountrySetupPage.CostDataType.NUMERIC.toString()) {
                
                tmkCECountrySetupPage.hideAllElements(elements);
    ...
    childGridEdit(e) {
        ...
        var elements = {
            ...
            tmkCCPICostLabel: { divElement: editorContainer.find('#tmkCCPiCostLabel') }
        };
        ...
        cDataTypeDropDownList.bind("change", function(event) {
            tmkCECountrySetupPage.childGridEditorDataTypeChange({
                data: {
                    sender: event.sender,                    
                    editorContainer: editorContainer
                }
        ...
        if (model.CCPICost && !tmkCECountrySetupPage.isCPIAdmin) {            
            elements.tmkCCPICostLabel.divElement.show();
            editorContainer.find('input, select, textarea').attr('disabled', 'disabled');
    ...
    childGridSave(e) {
        ...
        if (e.model.CDataType != tmkCECountrySetupPage.CostDataType.STRING && (e.model.CDefaultValue === "" || e.model.CDefaultValue === null)) {
            e.preventDefault();
            alert('Default value is required.');
        }
    ...
    childGridDataBound(e) {
        ...
        grid.tbody.find("tr[role='row']").each(function () {
            ...
            if (row.CDataType == tmkCECountrySetupPage.CostDataType.NUMERIC) {
                var cell = $(this).children().eq(columnIndex);
                cell.html(row.COpts + " " + row.CDefaultValue);
            }
            else if (row.CDataType == tmkCECountrySetupPage.CostDataType.NUMERICRANGE) {
                var cell = $(this).children().eq(columnIndex);
                if (row.CAltValue) {
            ...
            if (!(row.CDataType == tmkCECountrySetupPage.CostDataType.STRING)) {
                $('tr[data-uid="' + row.uid + '"] ').find(".k-hierarchy-cell a").remove();
            }

            //Persist expanded rows            
            if (tmkCECountrySetupPage.childGridExpandedRows) {                
                if (row) {                    
                    if ($.inArray(row.CCId, tmkCECountrySetupPage.childGridExpandedRows) >= 0) {                        
    ...
    childGridDetailExpand(e) {
        let dataItem = this.dataItem(e.masterRow);
        if (dataItem) {
            tmkCECountrySetupPage.childGridExpandedRows.push(dataItem.CCId);
        }
    }
    ...
    childGridDetailCollapse(e) {
        let dataItem = this.dataItem(e.masterRow);
        if (dataItem) {
            tmkCECountrySetupPage.childGridExpandedRows = tmkCECountrySetupPage.childGridExpandedRows.filter(id => id !== dataItem.CCId);
        }
    }
    ...
    subGridEdit(e) {
        ...
        var elements = {
            ...
            tmkSCPICostLabel: { divElement: editorContainer.find('#tmkSCPiCostLabel'), input: "" },
        ...
        if (model.SCPICost && !tmkCECountrySetupPage.isCPIAdmin) {
            elements.tmkSCPICostLabel.divElement.show();
            editorContainer.find('input, select, textarea').attr('disabled', 'disabled');
        ...
        sDataTypeDropDownList.bind("change", function(event) {
            tmkCECountrySetupPage.subGridEditorDataTypeChange({
                data: {
                    sender: event.sender,
        ...
        //// Bind and trigger for the Description input
        //var descriptionInput = editorContainer.find("#SDescription");
        //var costFactor1NumericTextBox = editorContainer.find("#SCostFactor1").data("kendoNumericTextBox");
        //var costFactor1Divs = editorContainer.find(".costFactor1Div");
    
        //descriptionInput.on("input", {
        //    model: model,
        //    costFactor1NumericTextBox: costFactor1NumericTextBox,
        //    costFactor1Divs: costFactor1Divs,
        //    descriptionInput: descriptionInput
        //}, tmkCECountrySetupPage.subGridEditorSDescriptionChange);
        ...
        // Bind TranslationType input change handler
        elements.translationType.input.bind("change", function(event) {
            tmkCECountrySetupPage.subGridEditorSTranslationTypeChange({
                data: {
                    selectedType: event.sender.text(),
                    model: model,
                    costFactor1NumericTextBox: elements.costFactor1.input,
        ...
        // Initial call to set translation type handler state
        const currentSTranslationType = model.STranslationType == tmkCECountrySetupPage.TranslationType.PAGES.Value ? tmkCECountrySetupPage.TranslationType.PAGES.Text : model.STranslationType == tmkCECountrySetupPage.TranslationType.WORDS.Value ? tmkCECountrySetupPage.TranslationType.WORDS.Text : tmkCECountrySetupPage.TranslationType.PAGES.Text;        
        tmkCECountrySetupPage.subGridEditorSTranslationTypeChange({
            data: {
    ...
    subGridSave(e) {        
        ...
        } else if (model.SDataType != tmkCECountrySetupPage.CostDataType.STRING && (model.SDefaultValue === "" || model.SDefaultValue === null)) {
            e.preventDefault();
            alert('Default value is required.');
        }
    ...
    subGridDataBound(e) {
        ...
        grid.tbody.find("tr[role='row']").each(function () {
            ...
            if (row.SDataType == tmkCECountrySetupPage.CostDataType.NUMERIC) {
                var cell = $(this).children().eq(columnIndex);
    ...
    nestedGridRequestEnd(e) {
        if(e.type == 'update' || e.type == 'create' || e.type == 'destroy') {
            tmkCECountrySetupPage.updateRecordStamps(); 
            this.read(); 
            pageHelper.showSuccess(e.response.success); 
            
            var parentGrid = $('#tmkCECountryCostGrid').data('kendoGrid');
