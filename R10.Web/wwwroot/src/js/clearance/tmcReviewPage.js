...
export default class TmcReviewPage extends SearchPage {
    ...
    showRemarksEditor = (row) => {
        ...
        const inputId = `remarks-${item.TmcId}`;
        const popUpContent = `
    ...
    showStatusEditor = (row, editor) => {
        ...
        if (url) {
            ...
            cpiConfirm.save(this.popUpTitle(item), editor,
                function () {
                    const clearanceStatus = $("#ClearanceStatus_tmcReviewPage").data('kendoComboBox').value();                    
