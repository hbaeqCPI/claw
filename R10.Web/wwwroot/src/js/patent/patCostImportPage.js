export default class PatCostImportPage {
        ...
        $(document).ready(() => {
            ...
            option.on("click", "a", function () {
                ...
                if (selection === "new") {
                    ...
                    const url = `${baseUrl}/Patent/CostTrackingImport`;
                    window.location = url;
                }
                else {
    ...
    initializeStep_FileUpload_FromHistory = (importId) => {
        const baseUrl = $("body").data("base-url");
        const url = `${baseUrl}/Patent/CostTrackingImport/GetImportJob`;
    ...
    initializeStep_Mapping = (next, importId) => {
        const baseUrl = $("body").data("base-url");
        const url = `${baseUrl}/Patent/CostTrackingImport/LoadStep_FieldMapping`;
    ...
    initializeStep_Review = (next, importId,onSuccess) => {
        const baseUrl = $("body").data("base-url");
        const url = `${baseUrl}/Patent/CostTrackingImport/LoadStep_Review`;
        ...
        $.get(url, { importId: importId })
            .done((result) => {
                ...
                importButton.on("click", () => {
                    ...
                    const url = `${baseUrl}/Patent/CostTrackingImport/Import`;                    
                    cpiLoadingSpinner.show();
    ...
    step3StatusCheck = (baseUrl, importId, next) => {
        const url = `${baseUrl}/Patent/CostTrackingImport/GetImportStatus`;                    
    ...
    initializeStep_ImportResult = (next, importId, success) => {
        ...
        const url = `${baseUrl}/Patent/CostTrackingImport/LoadStepImportResult_${status}`;
        $.get(url, { importId: importId }).done((result) => {
            next.html(result);
            if (!success) {
    ...
    stepImportResult_Processing = (baseUrl, importId, next) => {
        const url = `${baseUrl}/Patent/CostTrackingImport/LoadStepImportResult_Processing`;
        $.get(url, { importId: importId }).done((result) => {
            next.html(result);
