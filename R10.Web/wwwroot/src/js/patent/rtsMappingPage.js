export default class RTSMappingPage {
    constructor() {
        this.mappingType = "IFWDocuments";
    }

    initializeUpdate = () => {
        this.loadUpdateScreen("IFWDocuments");
        this.loadUpdateScreen("TransactionHistory");
    }

    initializeTabs = () => {
        $("#rtsMappingMain-tab a").off("click");
        $("#rtsMappingMain-tab a").on("click", (e) => {
            //expand search criteria tab
            const tab = $(".rts-mapping-main .refine-search .nav-link");
            tab.addClass("show");
            tab.siblings(".accordion-content").addClass("show").css("display", "block");

            //sync horizontal tabs
            var href = $(e.currentTarget).attr('href');
            $('.nav-tabs#rtsMappingMain-tab li a').removeClass('active');
    ...
    loadUpdateScreen(mappingType) {
        ...
        const url = `${baseUrl}/Patent/RTSMapping/GetMappingScreen`;
    ...
    initializeIFWDocumentsMappingGrids = () => {
        const topGrid = $('#rtsIFWDocumentsMapping-Grid');
        const el = {
            name: 'rtsIFWDocumentsMapping-Grid', isDirty: false
        };
        ...
        topGrid.on("click", ".client-entry", (e) => {
            ...
            $.get(url, {mapId:currentRow.MapId}).done(function (result) {
                ...
                const clientEntryGrid = $('#rtsMapActionDocumentClientGrid');
                const el = {
                    name: 'rtsMapActionDocumentClientGrid', isDirty: false
                };
    ...
    onIFWDocumentsSourceRowSelect = (row) => {
        const grid = $('#rtsIFWDocumentsMapping-Grid');
    ...
    onTrnxHistorySourceRowSelect = (row) => {
        const grid = $('#rtsTransactionHistoryMapping-Grid');
    ...
    onTrnxHistoryMappedActionRowSelect = (row) => {
        const data = row.dataItem(row.select());
        $("#trnxHistoryPTOMappingActionType").val(data.PMSActionType);
    }
