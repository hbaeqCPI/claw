...
export default class FFPortfolioReviewPage extends ActivePage {
    ...
    checkAllCountries = (e) => {
        const el = $(e.target);
        $(`.${el.attr("value")} input:not(:disabled)`).prop("checked", el.prop("checked"));
    }

    toggleEP = (e) => {
        this.toggleParentCountry($(e.target), $("#all-EP"));
    }

    toggleWO = (e) => {
        this.toggleParentCountry($(e.target), $("#all-WO"));
    }

    toggleParentCountry = (el, country) => {
        const anyChecked = el.parent().parent().find("input:checkbox:checked").length > 0;

        country.prop("disabled", anyChecked);
        if (anyChecked)
            country.parent().find(".label-message").hide();
        else
            country.parent().find(".label-message").show();
    }

    checkCountries = (e) => {
        const el = $(e.target);
        const tab = el.data("des-ctry-tab");

        if (tab) {
            if (el.prop("checked")) {
                $(`.${tab}`).show();
                el.parent().find(".label-message").show();
            }
            else {
                $(`.${tab}`).hide();
                el.parent().find(".label-message").hide();
            }

            const tabStrip = $(`.${tab}`).parent();
            tabStrip.find("li").removeClass("last");
            tabStrip.find("li:visible:last").addClass("last");
        }

        const generatedMessage = el.parent().find(".generated-message");
        if (generatedMessage) {
            if (el.prop("checked")) {
                generatedMessage.hide();
            }
            else {
                generatedMessage.show();
            }
        }
    }
    ...
    saveInstruction = (instruction, row, selectCountries, edit) => {
        ...
        const save = (ctryEP, ctryWO, ctryAll, reason) => {
            cpiLoadingSpinner.show();
        ...
        const saveReason = (ctryEP, ctryWO, ctryAll) => {
            ...
            cpiConfirm.save(this.popUpTitle(item), `
                ...
                function () {
                    ...
                    if (reasonForChange === "") {
                        ...
                        save(ctryEP, ctryWO, ctryAll, reasonForChange);
                ...
                $.post(getCountryUrl, data)
                    .done((result) => {
                        ...
                        cpiConfirm.save(this.popUpTitle(item), result,
                            function () {
                                if ($('.country-check input:checked').length == 0) {
                                    ...
                                    $('.country-check-ep input:checked').each(function () {
                                        ctryEP.push($(this).attr('value'));
                                    });

                                    $('.country-check-wo input:checked').each(function () {
                                        ctryWO.push($(this).attr('value'));
                                    });

                                    $('.country-check-all input:checked').each(function () {
                                        ctryAll.push($(this).attr('value'));
                                    });

                                    //console.log(ctryEP, ctryWO, ctryAll);

                                    if (needsReasonForChange && clientInstructionType) {
                                        cpiConfirm.keepOpen = true;
                                        saveReason(ctryEP, ctryWO, ctryAll);
                                    }
                                    else
                                        save(ctryEP, ctryWO, ctryAll, item.ClientInstructionType ? tickler.data("reason") : "");
                                }
                            }, true,
                            function () {
                                undo();
    ...
    popUpTitle(item) {
        return `<div class="row mt-1">
                ...
                <div class="ff-modal-status page-status alert alert-danger collapse" role="alert"><span class="message"></span></div>`;
    }
    ...
    popUpHeader(row) {
        ...
        return `<div class="form-row">
                    <div class="col">
                        <div class="form-group float-label"><label>${tickler.data("label-action-due")}</label><input type="text" class="form-control form-control-sm" value="${item.ActionDue}" disabled></input></div>
                    </div>
                    <div class="col">
    ...
    showDocUploader = (row, docId) => {
        ...
        const item = this.searchResultGrid.data("kendoListView").dataItem(row);
        this.docUploader(form, item, docId);
    }

    docUploader = (form, item, docId) => {
        const url = form.data("upload-doc-url");

        if (url.toLowerCase().includes("imanagework"))
            iManage.saveDocumentFF(this, url, item, docId)
        else if (url.toLowerCase().includes("netdocs"))
            docViewer.saveDocumentFF(this, url, item, docId)
        else if (url) {
            this.image.container = $(this.searchResultContainer);
            this.image.hasViewScreen = false;
            this.image.openPopEditor(`${url}?documentLink=P|Act|ActId|${item.ActId}&dueId=${item.DueId}&docId=${docId || 0}`)
                .fail((e) => {
                    const spTokenUrl = form.data("sp-token-url");
                    if (e.status == 401 && spTokenUrl) {
                        sharePointGraphHelper.getGraphToken(spTokenUrl, () => {
                            this.docUploader(form, item, docId);
                        });
                    }
                    else {
                        cpiLoadingSpinner.hide();
                        cpiAlert.warning(pageHelper.getErrorMessage(e));
                    }
                })
                .then(() => {
                    this.refreshPage();
                ...
                .done((result) => {
                    ...
                    setTimeout(() => {
                        $(".doc-management .doc-upload").on("click", (e) => {
                            ...
                            cpiAlert.close();
                            this.docUploader(form, item, dueDoc.data("doc-id"));
                        });
            ...
            $.get(url, { ddId: ddId })
                .done(function (result) {
                    ...
                    for (const country of result) {
                        const chk = $(`#all-${country}:not(:disabled)`);
