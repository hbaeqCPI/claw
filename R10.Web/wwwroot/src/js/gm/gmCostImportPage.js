export default class GMCostImportPage {
        ...
        $(document).ready(() => {
            ...
            option.on("click", "a", function () {
                ...
                if (selection === "new") {
                    ...
                    const url = `${baseUrl}/GeneralMatter/CostTrackingImport`;
                    window.location = url;
                }
                else {
                    const grid = $("#costTrackingImportHistoryGrid");
                    grid.data("kendoGrid").dataSource.read();
                }
            });

            $("#costTracking-import-main").on("click", "#getStructure", function (e) {
                e.preventDefault();
                const url = $(this).attr("href");
                $.get(url).done(function (result) {
                    const popupContainer = $(".cpiContainerPopup").last();
                    popupContainer.html(result);
                    const dialogContainer = popupContainer.find("#costTrackingImportTypeColumnsDialog");
                    dialogContainer.modal("show");
                });
            });

            $("#costTracking-import").on("click", ".details-link", function (e) {
                e.preventDefault();
                const link = $(this);
    ...
    initializeStep_FileUpload_FromHistory = (importId) => {
        const baseUrl = $("body").data("base-url");
        const url = `${baseUrl}/GeneralMatter/CostTrackingImport/GetImportJob`;
    ...
    initializeStep_Mapping = (next, importId) => {
        const baseUrl = $("body").data("base-url");
        const url = `${baseUrl}/GeneralMatter/CostTrackingImport/LoadStep_FieldMapping`;
    ...
    initializeStep_Review = (next, importId,onSuccess) => {
        const baseUrl = $("body").data("base-url");
        const url = `${baseUrl}/GeneralMatter/CostTrackingImport/LoadStep_Review`;
        ...
        $.get(url, { importId: importId })
            .done((result) => {
                ...
                importButton.on("click", () => {
                    ...
                    const url = `${baseUrl}/GeneralMatter/CostTrackingImport/Import`;                    
                    cpiLoadingSpinner.show();
    ...
    step3StatusCheck = (baseUrl, importId, next) => {
        const url = `${baseUrl}/GeneralMatter/CostTrackingImport/GetImportStatus`;                    
    ...
    initializeStep_ImportResult = (next, importId, success) => {
        ...
        const url = `${baseUrl}/GeneralMatter/CostTrackingImport/LoadStepImportResult_${status}`;
        $.get(url, { importId: importId }).done((result) => {
            next.html(result);
            if (!success) {
    ...
    stepImportResult_Processing = (baseUrl, importId, next) => {
        const url = `${baseUrl}/GeneralMatter/CostTrackingImport/LoadStepImportResult_Processing`;
        $.get(url, { importId: importId }).done((result) => {
            next.html(result);
