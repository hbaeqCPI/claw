...
namespace R10.Web.Areas.Trademark.Controllers
{
    ...
    public class DesCaseTypeExtController : BaseController
    {
        ...
        {
            var data = await _repository.TmkDesCaseTypeExts.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Des Case Type Ext"].ToString(), Data = new TmkDesCaseTypeExt() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.TmkDesCaseTypeExts.AsNoTracking().ToListAsync();
            if (id < 0 || id >= items.Count) return new RecordDoesNotExistResult();
            var detail = items[id];
            var model = new PageViewModel { Page = PageType.Detail, PageId = _dataContainer, Title = _localizer["Des Case Type Ext Detail"].ToString(), RecordId = id, SingleRecord = singleRecord || !Request.IsAjax(), Data = detail };
        ...
        public async Task<IActionResult> Save([FromBody] TmkDesCaseTypeExt entity)
        {
            ...
            _repository.Set<TmkDesCaseTypeExt>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = TrademarkAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] TmkDesCaseTypeExt entity)
        {
            _repository.Set<TmkDesCaseTypeExt>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
