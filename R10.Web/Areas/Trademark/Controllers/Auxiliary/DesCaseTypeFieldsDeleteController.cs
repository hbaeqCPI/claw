...
namespace R10.Web.Areas.Trademark.Controllers
{
    ...
    public class DesCaseTypeFieldsDeleteController : BaseController
    {
        ...
        {
            var data = await _repository.TmkDesCaseTypeFieldsDeletes.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Des Case Type Fields Delete"].ToString(), Data = new TmkDesCaseTypeFieldsDelete() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.TmkDesCaseTypeFieldsDeletes.AsNoTracking().ToListAsync();
        ...
        public async Task<IActionResult> Save([FromBody] TmkDesCaseTypeFieldsDelete entity)
        {
            ...
            _repository.Set<TmkDesCaseTypeFieldsDelete>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = TrademarkAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] TmkDesCaseTypeFieldsDelete entity)
        {
            _repository.Set<TmkDesCaseTypeFieldsDelete>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
