...
namespace R10.Web.Areas.Trademark.Controllers
{
    ...
    public class DesCaseTypeFieldsDeleteExtController : BaseController
    {
        ...
        {
            var data = await _repository.TmkDesCaseTypeFieldsDeleteExts.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Des Case Type Fields Delete Ext"].ToString(), Data = new TmkDesCaseTypeFieldsDeleteExt() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.TmkDesCaseTypeFieldsDeleteExts.AsNoTracking().ToListAsync();
        ...
        public async Task<IActionResult> Save([FromBody] TmkDesCaseTypeFieldsDeleteExt entity)
        {
            ...
            _repository.Set<TmkDesCaseTypeFieldsDeleteExt>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = TrademarkAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] TmkDesCaseTypeFieldsDeleteExt entity)
        {
            _repository.Set<TmkDesCaseTypeFieldsDeleteExt>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
