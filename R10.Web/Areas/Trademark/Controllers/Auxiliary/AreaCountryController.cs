...
namespace R10.Web.Areas.Trademark.Controllers
{
    ...
    public class AreaCountryController : BaseController
    {
        ...
        {
            var data = await _repository.TmkAreasCountries.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Area Country"].ToString(), Data = new TmkAreaCountry() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.TmkAreasCountries.AsNoTracking().ToListAsync();
            if (id < 0 || id >= items.Count) return new RecordDoesNotExistResult();
            var detail = items[id];
            var model = new PageViewModel { Page = PageType.Detail, PageId = _dataContainer, Title = _localizer["Area Country Detail"].ToString(), RecordId = id, SingleRecord = singleRecord || !Request.IsAjax(), Data = detail };
        ...
        public async Task<IActionResult> Save([FromBody] TmkAreaCountry entity)
        {
            ...
            _repository.Set<TmkAreaCountry>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = TrademarkAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] TmkAreaCountry entity)
        {
            _repository.Set<TmkAreaCountry>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
