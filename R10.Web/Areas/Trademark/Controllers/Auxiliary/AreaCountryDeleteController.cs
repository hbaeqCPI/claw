...
namespace R10.Web.Areas.Trademark.Controllers
{
    ...
    public class AreaCountryDeleteController : BaseController
    {
        ...
        {
            var data = await _repository.TmkAreaCountryDeletes.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Area Country Delete"].ToString(), Data = new TmkAreaCountryDelete() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.TmkAreaCountryDeletes.AsNoTracking().ToListAsync();
            if (id < 0 || id >= items.Count) return new RecordDoesNotExistResult();
            var detail = items[id];
            var model = new PageViewModel { Page = PageType.Detail, PageId = _dataContainer, Title = _localizer["Area Country Delete Detail"].ToString(), RecordId = id, SingleRecord = singleRecord || !Request.IsAjax(), Data = detail };
        ...
        public async Task<IActionResult> Save([FromBody] TmkAreaCountryDelete entity)
        {
            ...
            _repository.Set<TmkAreaCountryDelete>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = TrademarkAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] TmkAreaCountryDelete entity)
        {
            _repository.Set<TmkAreaCountryDelete>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
