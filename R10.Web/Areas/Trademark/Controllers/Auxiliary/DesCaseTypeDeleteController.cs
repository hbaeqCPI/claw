...
namespace R10.Web.Areas.Trademark.Controllers
{
    ...
    public class DesCaseTypeDeleteController : BaseController
    {
        ...
        {
            var data = await _repository.TmkDesCaseTypeDeletes.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Des Case Type Delete"].ToString(), Data = new TmkDesCaseTypeDelete() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.TmkDesCaseTypeDeletes.AsNoTracking().ToListAsync();
        ...
        public async Task<IActionResult> Save([FromBody] TmkDesCaseTypeDelete entity)
        {
            ...
            _repository.Set<TmkDesCaseTypeDelete>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = TrademarkAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] TmkDesCaseTypeDelete entity)
        {
            _repository.Set<TmkDesCaseTypeDelete>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
