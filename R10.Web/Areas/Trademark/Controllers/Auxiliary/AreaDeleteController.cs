...
namespace R10.Web.Areas.Trademark.Controllers
{
    ...
    public class AreaDeleteController : BaseController
    {
        ...
        {
            var data = await _repository.TmkAreaDeletes.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Area Delete"].ToString(), Data = new TmkAreaDelete() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.TmkAreaDeletes.AsNoTracking().ToListAsync();
        ...
        public async Task<IActionResult> Save([FromBody] TmkAreaDelete entity)
        {
            ...
            _repository.Set<TmkAreaDelete>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = TrademarkAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] TmkAreaDelete entity)
        {
            _repository.Set<TmkAreaDelete>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
