...
{
    public class PatImageActViewModelService : IPatImageActViewModelService
    {
        ...
        public PatImageActViewModelService(
        ...
        public async Task<List<DocDocumentListViewModel>> CreateViewModelForList(int parentId)
        {
            ...
            if (settings.IsDocumentVerificationOn)
            {
                docs = await _docService.DocDocuments.Where(d => _docService.DocFolders
                                        .Where(f => f.SystemType == SystemTypeCode.Patent && ((f.DataKey == "ActId" && f.DataKeyValue == parentId)
                                                || (f.DataKey == "AppId" && _repository.PatActionDues.Any(a => a.AppId == f.DataKeyValue && a.ActId == parentId)))
                                            )
                                .Any(f => f.FolderId == d.FolderId))
                                .ProjectTo<DocDocumentListViewModel>()
