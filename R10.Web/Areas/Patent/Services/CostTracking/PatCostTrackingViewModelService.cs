...
{
    public class PatCostTrackingViewModelService: IPatCostTrackingViewModelService
    {
        ...
        public PatCostTrackingViewModelService(ICostTrackingService<PatCostTrack> costTrackingService, ICountryApplicationService applicationService, IMapper mapper)
        {
        ...
        public IQueryable<PatCostTrack> AddCriteria(List<QueryFilterViewModel> mainSearchFilters, IQueryable<PatCostTrack> costTracks)
        {
            if (mainSearchFilters.Count > 0)
            {
                ...
                if (caseType != null)
                {
                    ...
                    if (caseTypes.Count > 0)
                    {
                        if (caseType.Operator == "eq")
                            costTracks = costTracks.Where(c => caseTypes.Contains(c.CountryApplication.CaseType));
                        else
                            costTracks = costTracks.Where(c => !caseTypes.Contains(c.CountryApplication.CaseType));
                ...
                if (mainSearchFilters.Any())
                    costTracks = QueryHelper.BuildCriteria<PatCostTrack>(costTracks, mainSearchFilters);
            }

            return costTracks;
        }

        public async Task<CPiDataSourceResult> CreateViewModelForGrid(DataSourceRequest request, IQueryable<PatCostTrack> costTracks)
        {
            var model = costTracks.ProjectTo<PatCostTrackingSearchResultViewModel>();
        ...
        public async Task<PatCostTrackingDetailViewModel> CreateViewModelForDetailScreen(int id)
        {
            var viewModel = new PatCostTrackingDetailViewModel();

            if (id > 0)
            {
                viewModel = await _costTrackingService.QueryableList.ProjectTo<PatCostTrackingDetailViewModel>()
        ...
        public PatCostTrack ConvertViewModelToCostTracking(PatCostTrackingDetailViewModel viewModel)
        {
            return _mapper.Map<PatCostTrack>(viewModel);
        }

        public async Task<CaseNumberLookupViewModel> CaseNumberSearchValueMapper(IQueryable<PatCostTrack> costTracks, string value)
        {
        ...
        public async Task<List<PatCostTrackingAppInfoViewModel>> GetAppInfoList(string caseNumber, string country, string subCase)
        {
            var appInfo = await _applicationService.CountryApplications
                .Where(c => c.CaseNumber == caseNumber)
                .ProjectTo<PatCostTrackingAppInfoViewModel>()
                .ToListAsync();

            return appInfo;
        }

        public IQueryable<PatCostTrackingAppInfoViewModel> AppInfo => _applicationService.CountryApplications.ProjectTo<PatCostTrackingAppInfoViewModel>();
    }
}
