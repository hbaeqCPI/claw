...
namespace R10.Web.Areas.Patent.Controllers
{
    ...
    public class DesCaseTypeDeleteController : BaseController
    {
        ...
        {
            var data = await _repository.PatDesCaseTypeDeletes.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Des Case Type Delete"].ToString(), Data = new PatDesCaseTypeDelete() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.PatDesCaseTypeDeletes.AsNoTracking().ToListAsync();
            if (id < 0 || id >= items.Count) return new RecordDoesNotExistResult();
            var detail = items[id];
            var model = new PageViewModel { Page = PageType.Detail, PageId = _dataContainer, Title = _localizer["Des Case Type Delete Detail"].ToString(), RecordId = id, SingleRecord = singleRecord || !Request.IsAjax(), Data = detail };
        ...
        public async Task<IActionResult> Save([FromBody] PatDesCaseTypeDelete entity)
        {
            ...
            _repository.Set<PatDesCaseTypeDelete>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = PatentAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] PatDesCaseTypeDelete entity)
        {
            _repository.Set<PatDesCaseTypeDelete>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
