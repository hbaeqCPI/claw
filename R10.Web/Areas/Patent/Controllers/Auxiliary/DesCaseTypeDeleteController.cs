...
namespace R10.Web.Areas.Patent.Controllers
{
    ...
    public class DesCaseTypeDeleteController : BaseController
    {
        ...
        private readonly string _dataContainer = "patDesCaseTypeDeleteDetail";
        ...
        {
            var model = new PageViewModel { Page = PageType.Search, PageId = "patDesCaseTypeDeleteSearch", Title = _localizer["Des Case Type Delete Search"].ToString(), CanAddRecord = (await _authService.AuthorizeAsync(User, PatentAuthorizationPolicy.AuxiliaryModify)).Succeeded };
        ...
        {
            var model = new PageViewModel { Page = PageType.SearchResults, PageId = "patDesCaseTypeDeleteSearchResults", Title = _localizer["Des Case Type Delete Search Results"].ToString(), CanAddRecord = (await _authService.AuthorizeAsync(User, PatentAuthorizationPolicy.AuxiliaryModify)).Succeeded };
        ...
        public async Task<IActionResult> PageRead([DataSourceRequest] DataSourceRequest request, List<QueryFilterViewModel> mainSearchFilters)
        {
            ...
            // Strip only bool filters; string "New" filters go through BuildCriteria.
            ...
            var baseRows = extFilter == "true"
                ...
                : await _repository.PatDesCaseTypeDeletes.AsNoTracking()
            ...
            var extRows = extFilter == "false"
                ? new List<DesCaseTypeDeleteSearchItem>()
                : await _repository.PatDesCaseTypeDeleteExts.AsNoTracking()
                    .Select(x => new DesCaseTypeDeleteSearchItem
                    {
                        IntlCode = x.IntlCode,
                        CaseType = x.CaseType,
        ...
        {
            var detail = await _repository.PatDesCaseTypeDeletes.AsNoTracking()
        ...
        public async Task<IActionResult> Save([FromBody] PatDesCaseTypeDelete entity)
        {
            ...
            if (!isNewRecord)
            {
                ...
                var existing = await _repository.PatDesCaseTypeDeletes.AsNoTracking()
                ...
                if (existing != null)
                {
                    ...
                    var allRecords = await _repository.PatDesCaseTypeDeletes.AsNoTracking()
                    ...
                    if (duplicates.Any())
                        return new JsonBadRequest($"The following systems are already assigned to another Des Case Type Delete record: {string.Join(", ", duplicates)}");

                    await _repository.Database.ExecuteSqlRawAsync(
                        @"UPDATE tblPatDesCaseTypeDelete SET IntlCode=@p0, CaseType=@p1, DesCountry=@p2, DesCaseType=@p3,
            ...
            else
            {
                ...
                await _repository.Database.ExecuteSqlRawAsync(
                    @"INSERT INTO tblPatDesCaseTypeDelete (IntlCode, CaseType, DesCountry, DesCaseType, [Default], IntlCodeNew, CaseTypeNew, DesCountryNew, DesCaseTypeNew, Systems)
        ...
        public async Task<IActionResult> Delete(string intlCode = "", string caseType = "", string desCountry = "", string desCaseType = "", string intlCodeNew = "", string caseTypeNew = "", string desCountryNew = "", string desCaseTypeNew = "", string systems = "")
        {
            var count = await _repository.Database.ExecuteSqlRawAsync(
                "DELETE FROM tblPatDesCaseTypeDelete WHERE IntlCode=@p0 AND CaseType=@p1 AND DesCountry=@p2 AND DesCaseType=@p3 AND Systems=@p8",
                new object[] {
        ...
        {
            return await GetPicklistData(_repository.PatDesCaseTypeDeletes.AsNoTracking(), request, property, text, filterType, requiredRelation);
        }
