...
namespace R10.Web.Areas.Patent.Controllers
{
    ...
    public class DesCaseTypeFieldsExtController : BaseController
    {
        ...
        {
            var data = await _repository.PatDesCaseTypeFieldsExts.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Des Case Type Fields Ext"].ToString(), Data = new PatDesCaseTypeFieldsExt() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.PatDesCaseTypeFieldsExts.AsNoTracking().ToListAsync();
        ...
        public async Task<IActionResult> Save([FromBody] PatDesCaseTypeFieldsExt entity)
        {
            ...
            _repository.Set<PatDesCaseTypeFieldsExt>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = PatentAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] PatDesCaseTypeFieldsExt entity)
        {
            _repository.Set<PatDesCaseTypeFieldsExt>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
