...
namespace R10.Web.Areas.Patent.Controllers
{
    ...
    public class AreaCountryDeleteController : BaseController
    {
        ...
        {
            var data = await _repository.PatAreaCountryDeletes.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Area Country Delete"].ToString(), Data = new PatAreaCountryDelete() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.PatAreaCountryDeletes.AsNoTracking().ToListAsync();
        ...
        public async Task<IActionResult> Save([FromBody] PatAreaCountryDelete entity)
        {
            ...
            _repository.Set<PatAreaCountryDelete>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = PatentAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] PatAreaCountryDelete entity)
        {
            _repository.Set<PatAreaCountryDelete>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
