...
namespace R10.Web.Areas.Patent.Controllers
{
    ...
    public class CountryExpDeleteController : BaseController
    {
        ...
        {
            var data = await _repository.PatCountryExpDeletes.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Country Exp Delete"].ToString(), Data = new PatCountryExpDelete() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.PatCountryExpDeletes.AsNoTracking().ToListAsync();
            if (id < 0 || id >= items.Count) return new RecordDoesNotExistResult();
            var detail = items[id];
            var model = new PageViewModel { Page = PageType.Detail, PageId = _dataContainer, Title = _localizer["Country Exp Delete Detail"].ToString(), RecordId = id, SingleRecord = singleRecord || !Request.IsAjax(), Data = detail };
        ...
        public async Task<IActionResult> Save([FromBody] PatCountryExpDelete entity)
        {
            ...
            _repository.Set<PatCountryExpDelete>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = PatentAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] PatCountryExpDelete entity)
        {
            _repository.Set<PatCountryExpDelete>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
