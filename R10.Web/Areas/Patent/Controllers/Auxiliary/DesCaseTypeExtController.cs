...
namespace R10.Web.Areas.Patent.Controllers
{
    ...
    public class DesCaseTypeExtController : BaseController
    {
        ...
        {
            var data = await _repository.PatDesCaseTypeExts.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Des Case Type Ext"].ToString(), Data = new PatDesCaseTypeExt() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.PatDesCaseTypeExts.AsNoTracking().ToListAsync();
        ...
        public async Task<IActionResult> Save([FromBody] PatDesCaseTypeExt entity)
        {
            ...
            _repository.Set<PatDesCaseTypeExt>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = PatentAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] PatDesCaseTypeExt entity)
        {
            _repository.Set<PatDesCaseTypeExt>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
