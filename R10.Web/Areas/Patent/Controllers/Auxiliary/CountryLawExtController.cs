...
namespace R10.Web.Areas.Patent.Controllers
{
    ...
    public class CountryLawExtController : BaseController
    {
        ...
        {
            var data = await _repository.PatCountryLawExts.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Country Law Ext"].ToString(), Data = new PatCountryLawExt() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.PatCountryLawExts.AsNoTracking().ToListAsync();
            if (id < 0 || id >= items.Count) return new RecordDoesNotExistResult();
            var detail = items[id];
            var model = new PageViewModel { Page = PageType.Detail, PageId = _dataContainer, Title = _localizer["Country Law Ext Detail"].ToString(), RecordId = id, SingleRecord = singleRecord || !Request.IsAjax(), Data = detail };
        ...
        public async Task<IActionResult> Save([FromBody] PatCountryLawExt entity)
        {
            ...
            _repository.Set<PatCountryLawExt>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = PatentAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] PatCountryLawExt entity)
        {
            _repository.Set<PatCountryLawExt>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
