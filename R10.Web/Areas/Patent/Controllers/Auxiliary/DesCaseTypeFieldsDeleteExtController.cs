...
namespace R10.Web.Areas.Patent.Controllers
{
    ...
    public class DesCaseTypeFieldsDeleteExtController : BaseController
    {
        ...
        private readonly string _dataContainer = "patDesCaseTypeFieldsDeleteExtDetail";
        ...
        {
            var model = new PageViewModel { Page = PageType.Search, PageId = "patDesCaseTypeFieldsDeleteExtSearch", Title = _localizer["Des Case Type Fields Delete Ext Search"].ToString(), CanAddRecord = (await _authService.AuthorizeAsync(User, PatentAuthorizationPolicy.AuxiliaryModify)).Succeeded };
        ...
        {
            var model = new PageViewModel { Page = PageType.SearchResults, PageId = "patDesCaseTypeFieldsDeleteExtSearchResults", Title = _localizer["Des Case Type Fields Delete Ext Search Results"].ToString(), CanAddRecord = (await _authService.AuthorizeAsync(User, PatentAuthorizationPolicy.AuxiliaryModify)).Succeeded };
        ...
        {
            var entities = _repository.PatDesCaseTypeFieldsDeleteExts.AsNoTracking().AsQueryable();
        ...
        {
            var detail = await _repository.PatDesCaseTypeFieldsDeleteExts.AsNoTracking()
        ...
        public async Task<IActionResult> Add(bool fromSearch = false, string copyDesCaseType = "", string copyFromField = "", string copyToField = "", string copyDesCaseTypeNew = "", string copyFromFieldNew = "", string copyToFieldNew = "", string copySystems = "")
        {
            ...
            var data = new PatDesCaseTypeFieldsDeleteExt { IsNewRecord = true };

            if (!string.IsNullOrEmpty(copyDesCaseType) || !string.IsNullOrEmpty(copyFromField))
            {
        ...
        public async Task<IActionResult> Save([FromBody] PatDesCaseTypeFieldsDeleteExt entity)
        {
            ...
            if (!isNewRecord)
            {
                ...
                var existing = await _repository.PatDesCaseTypeFieldsDeleteExts.AsNoTracking()
                ...
                if (existing != null)
                {
                    ...
                    var allRecords = await _repository.PatDesCaseTypeFieldsDeleteExts.AsNoTracking()
                    ...
                    if (duplicates.Any())
                        return new JsonBadRequest($"The following systems are already assigned to another Des Case Type Fields Delete Ext record: {string.Join(", ", duplicates)}");

                    await _repository.Database.ExecuteSqlRawAsync(
                        @"UPDATE tblPatDesCaseTypeFieldsDelete_Ext SET DesCaseType=@p0, FromField=@p1, ToField=@p2,
            ...
            else
            {
                ...
                await _repository.Database.ExecuteSqlRawAsync(
                    @"INSERT INTO tblPatDesCaseTypeFieldsDelete_Ext (DesCaseType, FromField, ToField, DesCaseTypeNew, FromFieldNew, ToFieldNew, Systems)
        ...
        public async Task<IActionResult> Delete(string desCaseType = "", string fromField = "", string toField = "", string desCaseTypeNew = "", string fromFieldNew = "", string toFieldNew = "", string systems = "")
        {
            var count = await _repository.Database.ExecuteSqlRawAsync(
                "DELETE FROM tblPatDesCaseTypeFieldsDelete_Ext WHERE DesCaseType=@p0 AND FromField=@p1 AND ToField=@p2 AND DesCaseTypeNew=@p3 AND FromFieldNew=@p4 AND ToFieldNew=@p5 AND Systems=@p6",
                new object[] {
        ...
        {
            if (!string.IsNullOrEmpty(desCaseType) || !string.IsNullOrEmpty(fromField))
