...
namespace R10.Web.Areas.Patent.Controllers
{
    ...
    public class AreaCountryController : BaseController
    {
        ...
        {
            var data = await _repository.PatAreasCountries.AsNoTracking().ToListAsync();
        ...
        public IActionResult Add(bool fromSearch = false)
        {
            ...
            var model = new PageViewModel { Page = fromSearch ? PageType.Detail : PageType.DetailContent, PageId = _dataContainer, Title = _localizer["New Area Country"].ToString(), Data = new PatAreaCountry() };
            ModelState.Clear();
            return PartialView("Index", model);
        }
        ...
        {
            var items = await _repository.PatAreasCountries.AsNoTracking().ToListAsync();
        ...
        public async Task<IActionResult> Save([FromBody] PatAreaCountry entity)
        {
            ...
            _repository.Set<PatAreaCountry>().Add(entity);
            await _repository.SaveChangesAsync();
            return Json(0);
        }

        [HttpPost, Authorize(Policy = PatentAuthorizationPolicy.AuxiliaryCanDelete), ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete([FromBody] PatAreaCountry entity)
        {
            _repository.Set<PatAreaCountry>().Remove(entity);
            await _repository.SaveChangesAsync();
            return Ok();
        }
