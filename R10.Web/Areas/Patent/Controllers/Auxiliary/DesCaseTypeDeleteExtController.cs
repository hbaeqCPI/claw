...
namespace R10.Web.Areas.Patent.Controllers
{
    ...
    public class DesCaseTypeDeleteExtController : BaseController
    {
        ...
        private readonly string _dataContainer = "patDesCaseTypeDeleteExtDetail";
        ...
        {
            var model = new PageViewModel { Page = PageType.SearchResults, PageId = "patDesCaseTypeDeleteExtSearchResults", Title = _localizer["Des Case Type Delete Ext Search Results"].ToString(), CanAddRecord = (await _authService.AuthorizeAsync(User, PatentAuthorizationPolicy.AuxiliaryModify)).Succeeded };
        ...
        public async Task<IActionResult> Delete(string intlCode = "", string caseType = "", string desCountry = "", string desCaseType = "", string intlCodeNew = "", string caseTypeNew = "", string desCountryNew = "", string desCaseTypeNew = "", string systems = "")
        {
            var count = await _repository.Database.ExecuteSqlRawAsync(
                "DELETE FROM tblPatDesCaseTypeDelete_Ext WHERE IntlCode=@p0 AND CaseType=@p1 AND DesCountry=@p2 AND DesCaseType=@p3 AND Systems=@p8",
                new object[] {
        ...
        {
            return await GetPicklistData(_repository.PatDesCaseTypeDeleteExts.AsNoTracking(), request, property, text, filterType, requiredRelation);
        }
