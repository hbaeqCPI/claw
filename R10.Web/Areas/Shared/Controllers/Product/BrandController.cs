...
namespace R10.Web.Areas.Shared.Controllers
{
    ...
    public class BrandController : BaseController
    {
        ...
        private readonly IEntityService<Brand> _auxService;
        private readonly IViewModelService<Brand> _viewModelService;
        private readonly IStringLocalizer<SharedResource> _localizer;
        private readonly IReportService _reportService;

        private readonly string _dataContainer = "Brand";

        public BrandController(
            IAuthorizationService authService,
            IEntityService<Brand> auxService,
            IViewModelService<Brand> viewModelService,
        ...
        public async Task<IActionResult> Index(string systemType = SystemTypeCode.Patent)
        {
            ...
            var model = new PageViewModel()
            {
                ...
                PageId = "BrandSearch",
                Title = _localizer["Brand Search"].ToString(),
        ...
        public async Task<IActionResult> Search([FromBody] List<QueryFilterViewModel> mainSearchFilters)
        {
            var model = new PageViewModel()
            {
                ...
                PageId = "BrandSearchResults",
                Title = _localizer["Brand Search Results"].ToString(),
                CanAddRecord = (await _authService.AuthorizeAsync(User, SharedAuthorizationPolicy.FullModify)).Succeeded
            };
        ...
        public async Task<IActionResult> PageRead([DataSourceRequest] DataSourceRequest request, List<QueryFilterViewModel> mainSearchFilters)
        {
            ...
            {
                var Brand = _viewModelService.AddCriteria(mainSearchFilters);
                var result = await _viewModelService.CreateViewModelForGrid(request, Brand, "BrandName", "BrandId");
                return Json(result);
            }
        ...
        public async Task<IActionResult> DetailLink(string id)
        {
            ...
            {
                var entity = await _viewModelService.GetEntityByCode("BrandName", id);
                if (entity == null)
                    return RedirectToAction(nameof(Add), new { Brand = id, fromSearch = true });
                else
                    return RedirectToAction(nameof(Detail), new { id = entity.BrandId, singleRecord = true, fromSearch = true });
            }
            else
            {
        ...
        public async Task<IActionResult> Detail(int id, bool singleRecord = false, bool fromSearch = false, string tab = "")
        {
            ...
            PageViewModel model = new PageViewModel()
            {
                ...
                Title = _localizer["Brand Detail"].ToString(),
                RecordId = detail.BrandId,
        ...
        public IActionResult Print()
        {
            ...
            ViewBag.DownloadName = "Brand Print Screen";
            return View();
        }

        [HttpPost]
        public IActionResult Print([FromBody] PrintViewModel sharedBrandPrintModel)
        {
            if (ModelState.IsValid)
            {
                return _reportService.GetReport(sharedBrandPrintModel, ReportType.SharedBrandPrintScreen).Result;
            }
        ...
        public async Task<IActionResult> Add(string Brand = "", bool fromSearch = false)
        {
            if (!Request.IsAjax())
                return RedirectToAction("Index");
            ...
            page.Detail.BrandName = Brand;
            var detail = page.Detail;
            PageViewModel model = new PageViewModel()
            {
                Page = fromSearch ? PageType.Detail : PageType.DetailContent,
                PageId = page.Container,
                Title = _localizer["New Brand"].ToString(),
                RecordId = detail.BrandId,
                PagePermission = page,
                Data = detail,
        ...
        public async Task<IActionResult> Save([FromBody] Brand Brand)
        {
            if (ModelState.IsValid)
            {
                UpdateEntityStamps(Brand, Brand.BrandId);

                if (Brand.BrandId > 0)
                    await _auxService.Update(Brand);
                else
                    await _auxService.Add(Brand);

                return Json(Brand.BrandId);
            }
            else
            {
                return new JsonBadRequest(new { errors = ModelState.Errors() });
        ...
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SaveRemarks([FromBody] Brand brand)
        {
            UpdateEntityStamps(brand, brand.BrandId);
            await _auxService.UpdateRemarks(brand);
            return Json(brand.BrandId);
        }

        public async Task<IActionResult> GetRecordStamps(int id)
        {
            var Brand = await _auxService.GetByIdAsync(id);
            if (Brand == null)
                return new NoRecordFoundResult();

            return ViewComponent("RecordStamps", new { Brand.CreatedBy, Brand.DateCreated, Brand.UpdatedBy, Brand.LastUpdate, Brand.tStamp });
        }

        private async Task<DetailPageViewModel<Brand>> PrepareEditScreen(int id)
        {
            var viewModel = new DetailPageViewModel<Brand>();
        ...
        private async Task<DetailPageViewModel<Brand>> PrepareAddScreen()
        {
            var viewModel = new DetailPageViewModel<Brand>();
            viewModel.Detail = new Brand();
        ...
        public async Task<IActionResult> GetBrandList([DataSourceRequest] DataSourceRequest request, string property, string text, FilterType filterType, string requiredRelation = "")
        {
            return await GetPicklistData(_auxService.QueryableList, request, property, text, filterType, new string[] { "BrandName", "BrandType" }, requiredRelation);
        }
    }
}