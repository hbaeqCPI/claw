...
-- The patent R8/R10 system tag rename ("PatR8-R10v2.1" -> "PatR8-10") is NOT here.
-- It is a data change rather than a schema change, it needs a report-then-apply pass,
-- and it needs a decision about the hist_ snapshots, so it lives in its own script:
--
--     rename-PatR8-10.sql   (repo root)
--
-- Run that once per database. It defaults to report-only.
