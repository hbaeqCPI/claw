...
{
    public interface ITmkActionDueApiService : IWebApiBaseService<TmkActionDueWebSvc, TmkActionDue>
    {
        IQueryable<TmkDueDate> DueDates { get; }
        Task<List<int>> AddDueDates(int actId, List<TmkDueDateWebSvc> webApiDueDates, DateTime runDate);
        Task UpdateDueDates(int actId, List<TmkDueDateWebSvc> webApiDueDates, DateTime runDate);
        Task LogApiDueDates(List<TmkDueDateWebSvc> webApiDueDates);
    }

    public class TmkActionDueApiService : WebApiBaseService<TmkActionDueWebSvc>, ITmkActionDueApiService
    {
        ...
        public IQueryable<TmkDueDate> DueDates => _dueDateService.QueryableList;

        IQueryable<TmkActionDue> IWebApiBaseService<TmkActionDueWebSvc, TmkActionDue>.QueryableList => _actionDueService.QueryableList;

        public TmkActionDueApiService(
        ...
        public async Task<int> Add(TmkActionDueWebSvc webApiActionDue, DateTime runDate)
        {
            await ValidateActionDue(0, webApiActionDue, true);
            return await SaveActionDue(webApiActionDue, new TmkActionDue(), runDate);
        }

        public async Task<List<int>> Import(List<TmkActionDueWebSvc> webApiActionsDue, DateTime runDate)
        {
            ...
            {
                actIds.Add(await SaveActionDue(webApiActionDue, new TmkActionDue(), runDate));
            }

            return actIds;
        }

        public async Task Update(int id, TmkActionDueWebSvc webApiActionDue, DateTime runDate)
        {
        ...
        public async Task Update(List<TmkActionDueWebSvc> webApiActionsDue, DateTime runDate)
        {
        ...
        public async Task<List<int>> AddDueDates(int actId, List<TmkDueDateWebSvc> webApiDueDates, DateTime runDate)
        {
            ...
            {
                ddIds.Add(await SaveDueDate(actId, webApiDueDate, new TmkDueDate(), runDate));
            }

            return ddIds;
        }

        public async Task UpdateDueDates(int actId, List<TmkDueDateWebSvc> webApiDueDates, DateTime runDate)
        {
        ...
        private async Task<int> SaveActionDue(TmkActionDueWebSvc webApiActionDue, TmkActionDue actionDue, DateTime runDate)
        {
        ...
        private async Task<int> SaveDueDate(int actId, TmkDueDateWebSvc webApiDueDate, TmkDueDate dueDate, DateTime runDate)
        {
        ...
        private async Task ValidateActionDue(int id, TmkActionDueWebSvc webApiActionDue, bool forInsert)
        {
            try
            {
        ...
        private async Task ValidateActionsDue(List<TmkActionDueWebSvc> webApiActionsDue, bool forInsert)
        {
        ...
        private async Task ValidateData(int id, TmkActionDueWebSvc webApiActionDue, bool forInsert)
        {
            //check key fields
            ...
            var actions = forInsert ?
                _cpiDbContext.GetReadOnlyRepositoryAsync<TmkActionDue>().QueryableList :
            ...
            if (forInsert)
            {
                ...
                Guard.Against.ValueNotAllowed(await _trademarkService.TmkTrademarks.AnyAsync(ca => ca.CaseNumber == webApiActionDue.CaseNumber && ca.Country == webApiActionDue.Country && ca.SubCase == webApiActionDue.SubCase), "Trademark");
                ...
                if (webApiActionDue.DueDates != null)
                {
                    ...
                    if (indicators.Any())
                        Guard.Against.ValueNotAllowed(indicators.Intersect(await _cpiDbContext.GetReadOnlyRepositoryAsync<TmkIndicator>().QueryableList.Select(i => i.Indicator).Distinct().ToListAsync()).Count() == indicators.Count, "Indicator");
        ...
        private async Task ValidateDueDates(int actId, List<TmkDueDateWebSvc> webApiDueDates, bool forInsert = false)
        {
            var errors = new List<string>();
        ...
        private async Task ValidateDueDateData(int actId, TmkDueDateWebSvc webApiDueDate, bool forInsert = false)
        {
            //check key fields
            ...
            var dueDates = forInsert ?
                _cpiDbContext.GetReadOnlyRepositoryAsync<TmkDueDate>().QueryableList :
            ...
            //check lookup fields
            Guard.Against.ValueNotAllowed(await _cpiDbContext.GetReadOnlyRepositoryAsync<TmkIndicator>().QueryableList.AnyAsync(i => i.Indicator == webApiDueDate.Indicator), "Indicator");
        ...
        private async Task SetData(TmkActionDueWebSvc webApiActionDue, TmkActionDue actionDue, DateTime runDate)
        {
        ...
        private async Task SetDueDateData(int actId, TmkDueDateWebSvc webApiDueDate, TmkDueDate dueDate, DateTime runDate)
        {
        ...
        public async Task LogApiDueDates(List<TmkDueDateWebSvc> webApiDueDates)
        {
            _cpiDbContext.GetRepository<TmkDueDateWebSvc>().Add(webApiDueDates);
